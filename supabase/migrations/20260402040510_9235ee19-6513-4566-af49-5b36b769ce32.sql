...
BEGIN
  ...
  recipient_total := COALESCE(array_length(target_message.recipients, 1), 0);

  next_delivered := COALESCE(_delivered_count,
    CASE
      WHEN normalized_status IN ('delivered', 'success', 'successful') THEN recipient_total
  ...
  next_failed := COALESCE(_failed_count,
    CASE
      WHEN normalized_status IN ('failed', 'undelivered', 'rejected') THEN recipient_total
  ...
  SET delivered_count = GREATEST(COALESCE(target_message.delivered_count, 0), COALESCE(next_delivered, 0)),
      ...
      status = CASE
        ...
        WHEN normalized_status IN ('sent', 'queued', 'pending') THEN 'sent'::public.message_status
